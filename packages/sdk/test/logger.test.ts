...
import { runCli } from "../../cli/src/run-cli.js";
import {
...
test("SDK events are queryable through the CLI", async () => {
  const root = createTempDir("logbook-sdk-cli-e2e-");
  const dbPath = join(root, "logs.db");
  const port = await getFreePort();
  const server = await startCollector({
    host: "127.0.0.1",
  ...
  try {
    ...
    const payload = rows[rows.length - 1]?.payloadJson
      ...
      : null;
    assert.equal(payload?.app, "sdk-cli-e2e");
